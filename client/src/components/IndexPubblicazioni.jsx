...
const IndexPubblicazioni = ({ onReadArticle }) => {
  ...
  // --- STESSA FUNZIONE DI SILLABAZIONE DI ARTICOLOSINGOLO ---
  const forceHyphenation = (text) => {
    if (!text) return "";
    // Trasforma spazi bloccanti in spazi normali
    let processed = text.replace(/&nbsp;/g, ' ');
    // Inserisce soft-hyphen (&shy;) in parole lunghe almeno 9 caratteri
    ...
    <div lang="it" style={{ maxWidth: "1200px", margin: "20px auto", padding: "0 20px", fontFamily: "Arial, sans-serif" }}>
      <style>{`
        @media (max-width: 992px) {
          .grid-evidenza { grid-template-columns: repeat(2, 1fr) !important; }
        ...
        @media (max-width: 600px) {
          .grid-evidenza { grid-template-columns: 1fr !important; }
      ...
      <div className="grid-evidenza" style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "20px", marginBottom: "40px" }}>
        {evidenza.map((a) => (
          <div key={a.id} style={{ padding: '15px', backgroundColor: '#fff', border: '1px solid #eee', borderRadius: '8px', display: 'flex', flexDirection: 'column' }}>
            ...
            {/* Anteprime brevi con sillabazione */}
            <p style={{ fontSize: '14px', color: '#666', marginBottom: '15px', textAlign: 'justify', hyphens: 'auto' }}>
              {extractText(a, 80)}
            </p>
      ...
      <div className="main-layout" style={{ display: "grid", gridTemplateColumns: "2fr 1fr", gap: "40px", borderTop: `3px solid ${colors.dark}`, paddingTop: "25px" }}>
        <section>
          ...
          <div 
            style={{ 
              ...
              marginBottom: '30px', 
              textAlign: 'justify', 
              textJustify: 'distribute', // Cambiato in distribute per uniformità
              hyphens: 'auto', 
              WebkitHyphens: 'auto',
          ...
          <button className="read-more-btn" onClick={() => onReadArticle(ultimoArticolo.id)} onMouseEnter={() => setIsHovered(true)} onMouseLeave={() => setIsHovered(false)} style={{ padding: '15px 35px', backgroundColor: isHovered ? "#333" : colors.dark, color: 'white', border: 'none', borderRadius: '4px', cursor: 'pointer', fontWeight: 'bold', fontSize: '16px', transition: 'all 0.3s ease', transform: isHovered ? 'scale(1.05)' : 'scale(1)', boxShadow: isHovered ? '0 4px 15px rgba(0,0,0,0.2)' : 'none' }}>
            Continua a leggere
          </button>
        </section>
        ...
        <aside className="sidebar-aside" style={{ borderLeft: `1px solid ${colors.border}`, paddingLeft: '30px' }}>
          ...
          {currentArchivioArt.length > 0 ? (
            <><ul style={{ listStyle: 'none', padding: 0 }}>{currentArchivioArt.map(a => (<li key={a.id} style={listItemStyle}><span onClick={() => onReadArticle(a.id)} style={{ cursor: 'pointer', fontSize: '14px', fontWeight: '600', color: '#333' }}>{a.titolo}</span></li>))}</ul><Pagination total={totalPagesArt} current={pageArticoli} setPage={setPageArticoli} /></>
          ...
          <div style={{ marginTop: '40px' }}>
            ...
            {ultimoSondaggio && (<div onClick={() => onReadArticle(ultimoSondaggio.id)} style={{ backgroundColor: colors.pollFocus, padding: '20px', borderRadius: '8px', color: 'white', cursor: 'pointer', marginBottom: '20px', transition: 'transform 0.2s' }} onMouseOver={e => e.currentTarget.style.transform='scale(1.02)'} onMouseOut={e => e.currentTarget.style.transform='scale(1)'}><h3 style={{ fontSize: '18px', margin: 0 }}>{ultimoSondaggio.titolo}</h3><p style={{ fontSize: '12px', marginTop: '10px', opacity: 0.8 }}>Vota ora →</p></div>)}
