    ...
    <div className="main-index-container" lang="it" style={{ maxWidth: "1180px", margin: "20px auto", padding: "0 20px", fontFamily: "Arial, sans-serif", position: "relative" }}>
      ...
      <div style={{ marginTop: "5%", marginBottom: "30px", display: "flex", justifyContent: "center", width: "100%", boxSizing: "border-box" }}>
        <input
          ...
          style={{ width: "100%", maxWidth: "400px", padding: "12px 20px", fontSize: "16px", borderRadius: "30px", border: `1px solid ${colors.border}`, transition: "all 0.3s", boxSizing: "border-box", margin: "0 auto" }}
        />
      </div>
      ...
      {searchTerm.trim() !== "" ? (
        <div style={{ minHeight: "60vh" }}>
          <h2 style={{ marginBottom: "20px", borderBottom: `2px solid ${colors.primary}`, paddingBottom: "10px" }}>Risultati per: "{searchTerm}" ({contenutiFiltrati.length})</h2>
          ...
          <button onClick={() => setSearchTerm("")} style={{ marginTop: "30px", background: "none", border: "none", color: colors.primary, cursor: "pointer", fontWeight: "bold" }}>← Torna alla home</button>
        </div>
      ) : (
        <>
          ...
          <div className="main-layout" style={{ display: "grid", gridTemplateColumns: "2fr 1fr", gap: "40px", borderTop: `3px solid ${colors.dark}`, paddingTop: "25px" }}>
            <section>
              <div className="mobile-rubriche">
                {(filtroCorrente === "HOME" || filtroCorrente === "RUBRICA") && (
                  <div style={{ backgroundColor: "#fff", border: `1px solid ${colors.border}`, padding: "20px", borderRadius: "8px" }}>
                    <h2 style={{ fontSize: '20px', borderBottom: `2px solid ${colors.rubriche}`, paddingBottom: '8px', marginBottom: '15px', marginTop: 0 }}>Le Rubriche</h2>
                    <div style={{ display: "flex", flexDirection: "column", gap: "2px" }}>
                      {listaRubriche.map((rub) => {
                        const isActive = rubricaAttiva === rub.key;
                        return <span key={rub.key} className="rubrica-item-link" style={rubricaLinkStyle(isActive)} onClick={() => { setRubricaAttiva(rub.key); setFiltroCorrente("RUBRICA"); setPageRubriche(1); }}>● {rub.label}</span>;
                      })}
                      {rubricaAttiva !== "" && <button onClick={() => { setRubricaAttiva(""); setFiltroCorrente("HOME"); }} style={{ marginTop: '10px', padding: '10px 12px', backgroundColor: '#6c757d', color: 'white', border: 'none', borderRadius: '4px', cursor: 'pointer', fontWeight: 'bold', fontSize: '12px', width: '100%' }}>❌ Rimuovi filtro rubrica</button>}
                    </div>
                  </div>
                )}
              </div>
              ...
              {filtroCorrente === "RUBRICA" && !ultimoContenutoPrincipale && (
                <div style={{ border: `1px dashed ${colors.border}`, padding: '40px 20px', borderRadius: '8px', marginBottom: '40px', textAlign: 'center', backgroundColor: colors.lightGray }}>
                  <div style={{ backgroundColor: colors.rubriche, color: 'white', display: 'inline-block', padding: '4px 12px', fontSize: '12px', fontWeight: 'bold', marginBottom: '15px', borderRadius: '2px', textTransform: "uppercase" }}>RUBRICA: {getNomeRubrica(rubricaAttiva)}</div>
              ...
              {ultimoEditoriale && (filtroCorrente === "HOME" || filtroCorrente === "EDITORIALI") && rubricaAttiva === "" && (
                <div style={{ border: `1px solid ${colors.editorial}`, padding: "25px", borderRadius: "8px", marginBottom: "40px", boxShadow: '0 4px 6px rgba(0,0,0,0.1)' }}>
                  <div style={{ backgroundColor: colors.editorial, color: 'white', display: 'inline-block', padding: '4px 12px', fontSize: '12px', fontWeight: 'bold', marginBottom: '15px', borderRadius: '2px' }}>EDITORIALE IN EVIDENZA</div>
                  <h2 style={{ fontSize: '32px', fontWeight: '700', marginBottom: '15px', lineHeight: '1.2', color: colors.dark }}>{ultimoEditoriale.titolo}</h2>
                  {ultimoEditoriale.sottotitolo && <p style={{ fontSize: '15px', color: '#555', margin: '0 0 15px 0', lineHeight: '1.4', fontStyle: 'italic' }}>{ultimoEditoriale.sottotitolo}</p>}
                  ...
                  {ultimoEditoriale.copertina && (
                    <div style={{ width: '100%', height: 'auto', maxHeight: '400px', backgroundColor: 'transparent', borderRadius: '2px', overflow: 'hidden', marginBottom: '20px', display: 'flex', justifyContent: 'center' }}>
                      <img src={ultimoEditoriale.copertina.startsWith('http') ? ultimoEditoriale.copertina : `data:image/jpeg;base64,${ultimoEditoriale.copertina}`} style={{ width: '100%', height: 'auto', display: 'block' }} alt="Editoriale Cover" />
                    </div>
                  )}
                  ...
                  <button className="editorial-btn" onClick={() => onReadArticle(ultimoEditoriale.id)} style={{ padding: '12px 30px', backgroundColor: colors.editorial, color: 'white', border: 'none', borderRadius: '4px', cursor: 'pointer', fontWeight: 'bold', fontSize: '15px', transition: 'all 0.3s ease', boxShadow: '0 4px 6px rgba(0,0,0,0.1)' }}>Continua a leggere</button>
                </div>
              )}
            ...
            <aside className="sidebar-aside" style={{ borderLeft: `1px solid ${colors.border}`, paddingLeft: '30px' }}>
              ...
              {(filtroCorrente === "HOME" || filtroCorrente === "RUBRICA") && (
                <div className="desktop-rubriche" style={{ marginBottom: "35px" }}>
                  <h2 style={{ fontSize: '20px', borderBottom: `2px solid ${colors.rubriche}`, paddingBottom: '8px', marginBottom: '15px' }}>Le Rubriche</h2>
                  <div style={{ display: "flex", flexDirection: "column", gap: "2px" }}>
                    {listaRubriche.map((rub) => {
                      const isActive = rubricaAttiva === rub.key;
                      return <span key={rub.key} className="rubrica-item-link" style={rubricaLinkStyle(isActive)} onClick={() => { setRubricaAttiva(rub.key); setFiltroCorrente("RUBRICA"); setPageRubriche(1); }}>● {rub.label}</span>;
                    })}
                    {rubricaAttiva !== "" && <button onClick={() => { setRubricaAttiva(""); setFiltroCorrente("HOME"); }} style={{ marginTop: '10px', padding: '6px 12px', backgroundColor: '#6c757d', color: 'white', border: 'none', borderRadius: '4px', cursor: 'pointer', fontWeight: 'bold', fontSize: '12px' }}>❌ Rimuovi filtro rubrica</button>}
                  </div>
                </div>
              )}
