...
const IndexPubblicazioni = ({ onReadArticle, onPrivacyClick }) => {
  ...
  const [selectedRubrica, setSelectedRubrica] = useState(""); // Stato per memorizzare la rubrica selezionata
  ...
  const contenutiPubblicati = tuttiContenuti.filter(
    item => item.bozza === false || item.bozza === 0 || item.bozza === "0"
  );

  // ESTRATTORE DINAMICO DELLE RUBRICHE PRESENTI NEL FILTRO DELLA TENDINA
  const elencoRubricheDisponibili = Array.from(
    new Set(
      contenutiPubblicati
        .map(c => c.rubrica?.trim())
        .filter(r => r && r !== "")
    )
  );
  ...
  return (
    <div lang="it" style={{ maxWidth: "1200px", margin: "20px auto", padding: "0 20px", fontFamily: "Arial, sans-serif", position: "relative" }}>
      <style>{`
        ...
        .editorial-btn:hover { transform: translateY(-3px) scale(1.02); box-shadow: 0 10px 20px rgba(0,0,0,0.15) !important; background-color: #5a32a3 !important; }
        .pagination-btn:hover { background-color: ${colors.lightGray} !important; border-color: ${colors.primary} !important; color: ${colors.primary} !important; transform: translateY(-1px); box-shadow: 0 4px 8px rgba(0,0,0,0.05); }
        .poll-card-main { transition: all 0.3s ease !important; }
        .poll-card-main:hover { transform: translateY(-3px) scale(1.02); box-shadow: 0 10px 20px rgba(0,0,0,0.2) !important; background-color: #0056b3 !important; }
        .banner-hover { transition: all 0.4s ease-in-out !important; cursor: pointer; }
        .banner-hover:hover { transform: scale(1.02); filter: brightness(1.1); box-shadow: 0 8px 25px rgba(0,0,0,0.15) !important; }
        .rubrica-card-hub { transition: all 0.3s ease; border: 1px solid #dee2e6; }
        .rubrica-card-hub:hover { transform: translateY(-2px); border-color: #17a2b8 !important; box-shadow: 0 5px 15px rgba(0,0,0,0.08); }
        .back-to-top-btn { position: fixed; bottom: 40px; right: calc(50% - 600px + 20px); z-index: 999; background-color: #007bff; color: white; border: none; padding: 12px 20px; font-weight: bold; border-radius: 50px; cursor: pointer; box-shadow: 0 4px 10px rgba(0, 0, 0, 0.15); display: flex; align-items: center; gap: 8px; font-size: 14px; transition: all 0.3s cubic-bezier(0.165, 0.84, 0.44, 1); opacity: 0; visibility: hidden; transform: translateY(20px); }
        .back-to-top-btn.visible { opacity: 1; visibility: visible; transform: translateY(0); }
        .back-to-top-btn:hover { background-color: #0056b3; transform: translateY(-3px); box-shadow: 0 6px 15px rgba(0, 56, 179, 0.3); }
        .back-to-top-btn:active { transform: translateY(0); }
        @media (min-width: 769px) and (max-width: 1024px) { .back-to-top-btn { right: 40px !important; } }
        @media (max-width: 768px) { .back-to-top-btn { right: 20px !important; bottom: 20px !important; padding: 10px 16px !important; font-size: 13px !important; } }
      `}</style>
      ...
      <div style={{ marginBottom: "15px", textAlign: "center" }}>
        <input
          ...
          style={{ width: "100%", maxWidth: "600px", padding: "12px 20px", fontSize: "16px", borderRadius: "30px", border: `1px solid ${colors.border}`, transition: "all 0.3s" }}
        />
      </div>

      {/* TENDINA DI SELEZIONE DELLE RUBRICHE */}
      <div style={{ marginBottom: "30px", textAlign: "center" }}>
        <select
          value={selectedRubrica}
          onChange={(e) => setSelectedRubrica(e.target.value)}
          style={{ width: "100%", maxWidth: "350px", padding: "10px 15px", fontSize: "14px", borderRadius: "8px", border: `1px solid ${colors.border}`, backgroundColor: "white", cursor: "pointer", fontWeight: "bold" }}
        >
          <option value="">Tutte le rubriche / Articoli generici</option>
          {elencoRubricheDisponibili.map((rubr, idx) => (
            <option key={idx} value={rubr}>{rubr.toUpperCase()}</option>
          ))}
        </select>
      </div>
      ...
      {searchTerm.trim() !== "" ? (
        <div style={{ minHeight: "60vh" }}>
            ...
            <div style={{ display: "grid", gap: "20px" }}>
              {contenutiFiltrati.map(item => (
                <div
                  ...
                  style={{ padding: "20px", backgroundColor: colors.lightGray, borderRadius: "8px", cursor: "pointer", display: "flex", justifyContent: "space-between", alignItems: "center", border: `1px solid ${colors.border}` }}
                >
                  <div>
          ...
          <button onClick={() => setSearchTerm("")} style={{ marginTop: "30px", background: "none", border: "none", color: colors.primary, cursor: "pointer", fontWeight: "bold" }}>
            ← Torna alla home
          </button>
        </div>
      ) : selectedRubrica !== "" ? (
        
        /* INIEZIONE COMPONENTE JSX ISOLATO: SE SELEZIONATA UNA RUBRICA, MOSTRA LA VISTA DEDICATA */
        <VisualizzaRubrica
          rubrica={selectedRubrica}
          tuttiContenuti={tuttiContenuti}
          onReadArticle={onReadArticle}
          onBack={() => setSelectedRubrica("")}
          getAutore={getAutore}
        />
        ...
        <>
            ...
            {evidenza.map((a) => (
              <div key={a.id} style={{ padding: '15px', backgroundColor: '#fff', border: '1px solid #eee', borderRadius: '8px', display: 'flex', flexDirection: 'column', boxShadow: '0 2px 4px rgba(0,0,0,0.02)' }}>
                <div style={{ width: '100%', height: '140px', backgroundColor: '#eee', borderRadius: '4px', overflow: 'hidden', marginBottom: '15px' }}>
                  {a.copertina && (
                    <img src={`data:image/jpeg;base64,${a.copertina}`} style={{ width: '100%', height: '100%', objectFit: 'cover', objectPosition: 'center' }} alt="Cover" />
                  )}
                </div>
                ...
                <span onClick={() => onReadArticle(a.id)} style={{ color: colors.primary, cursor: 'pointer', fontWeight: 'bold', fontSize: '13px', display: 'inline-block' }}>
                  Leggi →
                </span>
              </div>
            ))}
          ...
          <div className="main-layout" style={{ display: "grid", gridTemplateColumns: "2fr 1fr", gap: "40px", borderTop: `3px solid ${colors.dark}`, paddingTop: "25px" }}>
            <section>
              {ultimoArticolo.id && (
                <div style={{ border: `1px solid ${colors.border}`, padding: '20px', borderRadius: '8px', marginBottom: '40px' }}>
                  ...
                  <h1 className="main-title" style={{ fontSize: '32px', fontWeight: '700', marginBottom: '20px', lineHeight: '1.1' }}>{ultimoArticolo.titolo}</h1>
            ...
            <aside className="sidebar-aside" style={{ borderLeft: `1px solid ${colors.border}`, paddingLeft: '30px' }}>
              ...
              <div style={{ marginTop: '45px' }}>
                <h2 style={{ fontSize: '20px', borderBottom: `2px solid #17a2b8`, paddingBottom: '8px', marginBottom: '15px' }}>Altre Rubriche</h2>
