...
const ArticoloSingolo = ({ id, onBack }) => {
  ...
  const deviceId = useMemo(() => {
    ...
    const str =
      `${nav.userAgent}${nav.language}${screen.colorDepth}${screen.width}${screen.height}`;
  ...
  useEffect(() => {
    ...
    if (id) {
      ...
      fetch(`http://localhost:8096/api/pagine/${id}/view`, {
        ...
        .catch(err =>
          console.error("Errore contatore visite:", err)
        );
    ...
    fetch(`http://localhost:8096/api/pagine/${id}/vota`, {
      ...
      .then(async (res) => {
        ...
        if (res.status === 403) {
          ...
          // BLOCCO EDITORE DAL BACKEND
          if (
            testo &&
            testo.toLowerCase().includes("editori")
          ) {
              ...
              return (
                ...
                >
                  ...
                  {isEditore && (
                    ...
                    >
                      Gli editori non possono partecipare ai sondaggi.
                    </div>
                  )}
