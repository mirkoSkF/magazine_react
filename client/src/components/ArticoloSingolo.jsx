  ...
  return (
    <div style={{ backgroundColor: '#fff', minHeight: '100vh', fontFamily: 'Arial, Helvetica, sans-serif' }}>
      ...
      <nav style={{ padding: '20px', borderBottom: '1px solid #eee', maxWidth: '900px', margin: '0 auto' }}>
        <button 
          onClick={onBack} 
          style={{ background: 'none', border: 'none', color: '#007bff', fontWeight: 'bold', cursor: 'pointer', fontSize: '16px', fontFamily: 'Arial, sans-serif' }}
        >
          ← Torna al Magazine
        </button>
      </nav>
      ...
      <article style={{ maxWidth: '900px', margin: '40px auto', padding: '0 40px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', background: '#f8f9fa', padding: '20px', borderRadius: '8px', border: '1px solid #dee2e6', marginBottom: '30px' }}>
          <div style={{ position: 'relative' }}>
            {autore?.fotoProfilo ? (
              ...
              <div style={{ width: '80px', height: '80px', borderRadius: '50%', background: '#ccc', marginRight: '20px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 'bold', color: 'white', fontFamily: 'Arial, sans-serif' }}>
                {autore?.nome?.charAt(0)}{autore?.cognome?.charAt(0)}
              </div>
            )}
          </div>
          <div>
            <h3 style={{ margin: 0, fontSize: '20px', fontFamily: 'Arial, sans-serif' }}>{autore?.nome} {autore?.cognome}</h3>
            <p style={{ color: '#666', margin: '5px 0', fontSize: '14px', fontFamily: 'Arial, sans-serif' }}>
              Pubblicato il {new Date(articolo.dataPubblicazione).toLocaleDateString('it-IT')}
            </p>
          </div>
        </div>

        {/* TITOLO ARTICOLO - FORZATO IN ARIAL */}
        <h1 style={{ 
          fontSize: '42px', 
        ...
        {/* CONTENUTO HTML - FORZATO IN ARIAL */}
