...
const DashboardEditore = ({ onEdit, auth }) => {
  const [articoli, setArticoli] = useState([]);
  
  // Header dinamico basato sull'autenticazione dell'utente
  ...
  useEffect(carica, [auth]);

  const elimina = async (id) => {
    if (window.confirm("Sei sicuro di voler eliminare questo articolo?")) {
      try {
        const response = await fetch(`http://localhost:8096/api/pagine/${id}`, { 
          method: 'DELETE', 
          headers: authHeader 
        });
        if (response.ok) {
  ...
  return (
    <div style={{ padding: '40px', maxWidth: '1200px', margin: '0 auto' }}>
      <style>{`
        .btn-sf-primary:hover { background-color: ${colors.primary} !important; color: white !important; }
        .btn-sf-danger:hover { background-color: ${colors.danger} !important; color: white !important; }
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left' }}>
          ...
          <tbody>
            {articoli.length === 0 ? (
              ...
              articoli.map(a => (
                <tr key={a.id} style={rowStyle} onMouseOver={(e) => e.currentTarget.style.backgroundColor = colors.lightGray} onMouseOut={(e) => e.currentTarget.style.backgroundColor = 'transparent'}>
                  <td style={tdStyle}>
                  ...
                  <td style={{ ...tdStyle, textAlign: 'right' }}>
                    <button onClick={() => onEdit(a.id)} className="btn-sf-primary" style={actionBtnStyle(colors.primary)}>Gestisci</button>
                    <button onClick={() => elimina(a.id)} className="btn-sf-danger" style={actionBtnStyle(colors.danger)}>Elimina</button>
                  </td>
                </tr>
              ))
            )}
...
const actionBtnStyle = (color) => ({ background: 'transparent', color: color, border: `1px solid ${color}`, padding: '6px 15px', borderRadius: '4px', marginLeft: '10px', cursor: 'pointer', fontSize: '13px', fontWeight: '500' });
